...
impl<S> FromRequestParts<S> for Claims {
    ...
    async fn from_request_parts(parts: &mut Parts, _state: &S) -> Result<Self, Self::Rejection> {
        let auth_header = parts
            ...
            .ok_or_else(|| JrnlError::AuthenticationError(AuthenticationError::BadAuthenticationHeader))?;

        let claims = decode_jwt(auth_header)
            .map_err(|_| JrnlError::AuthenticationError(AuthenticationError::InvalidToken))?;

        if claims.exp < usize::try_from(Utc::now().timestamp()).map_err(Into::<anyhow::Error>::into)? {
